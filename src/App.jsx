...
enableIndexedDbPersistence(db).catch((err) => {
  if (err.code === 'failed-precondition') console.error("Multiple tabs open, persistence failed.");
  else if (err.code === 'unimplemented') console.error("Browser lacks persistence support.");
});
...
export default function App() {
  ...
  const [activeModal, setActiveModal] = useState(null); // 'personalList', 'city', 'detail', 'info', 'timing', 'accessDenied', 'contact'
  ...
  const [showSpecialPrayersEdit, setShowSpecialPrayersEdit] = useState(false);
  ...
  useEffect(() => {
    ...
    const unsubLocations = onSnapshot(doc(db, 'settings', 'locations'), (docSnap) => {
        if (docSnap.exists() && docSnap.data().cities) {
            setAvailableCities(new Set(docSnap.data().cities));
        }
    });
  ...
  const formatTime12 = (time24, pid = null) => { 
      ...
      hours = hours % 12 || 12; 
      return `${hours}:${String(m).padStart(2,'0')}<span class="text-[10px] ml-0.5 font-normal text-gray-400">${suffix}</span>`; 
  };
  ...
  const saveUserData = (newLists, newOrder) => {
      ...
      if (currentUser) {
          setDoc(doc(db, 'users', currentUser.uid), { personalLists: newLists, customOrder: newOrder || customOrder }, { merge: true });
      }
  };
  ...
  const togglePersonalList = (listName, id) => {
      ...
      if (newLists[listName].includes(id)) {
          newLists[listName] = newLists[listName].filter(x => x !== id);
      } else {
          newLists[listName].push(id);
      }
  ...
  const movePersonalOrder = (id, dir) => {
      ...
      if (!currentOrder.includes(id)) {
          currentOrder = mosques.map(m => m.id);
      }
      ...
      if (targetIdx >= 0 && targetIdx < currentOrder.length) {
          ...
          saveUserData(personalLists, currentOrder);
      }
  };
  ...
  const addNewCity = async () => {
      ...
      if(val) {
          ...
          setAppSettings(prev => ({ ...prev, city: formatted }));
          localStorage.setItem('city', formatted);
          setActiveModal(null);
          setNewCityInput('');
      }
  };
  ...
  const deleteMosque = async () => { 
      if(window.confirm("Delete this masjid?")) { 
          await deleteDoc(doc(db, 'mosques', selectedMosqueId)); 
          setActiveModal(null); 
  ...
  const openMosqueModal = (id = null) => {
      ...
      if (id) {
          ...
          setMosqueFormData({ name: 'Masjid-e-', area: '', locationLink: '', address: '' });
      }
      setActiveModal('info');
  };
  ...
  const openEditTiming = (id) => {
      ...
      if (m) {
          ...
          [...prayersList, ...specialPrayersList].forEach(p => {
              if (m.timings[p.id]) {
                  initTimings[p.id] = {
                      time: m.timings[p.id].time,
                      fixed: m.timings[p.id].fixed || false,
                      date: m.timings[p.id].lastUpdated ? m.timings[p.id].lastUpdated.split('T')[0] : new Date().toISOString().split('T')[0]
                  };
              } else {
                  initTimings[p.id] = { time: '', fixed: false, date: new Date().toISOString().split('T')[0] };
              }
          });
          setTimingFormData(initTimings);
      }
      setShowSpecialPrayersEdit(false);
  ...
  const renderNextPrayerMode = () => {
      if (currentList === 'Jummah') {
          ...
          return (
              <>
                  <div className="mt-4 mb-2 flex items-center gap-2 px-1"><i className="fas fa-users text-xs text-emerald-600"></i><h3 className="text-xs font-bold text-emerald-700 dark:text-emerald-400 uppercase tracking-widest">Jummah Timings</h3></div>
                  {jummaList.map(m => {
                      ...
                      return (
                          <div key={m.id} onClick={() => { setSelectedMosqueId(m.id); setActiveModal('detail'); }} className="flex justify-between items-center bg-white dark:bg-gray-800 px-4 py-2.5 rounded-xl shadow-sm border-l-[3px] border-emerald-600 mb-1 animate-card cursor-pointer">
                              <div className="flex-1 mr-4 flex items-center gap-2"><i className="fas fa-mosque text-[10px] text-emerald-500/30"></i><div><h4 className="font-bold text-sm dark:text-white leading-tight">{m.name}</h4><p className="font-bold text-[9px] text-gray-400 font-medium">{m.area}</p></div></div>
                              <div className="text-right font-mono font-bold text-lg text-emerald-700 dark:text-emerald-400">{parseInt(h)%12||12}:{mins}<span className="text-[9px] ml-1">{h>=12?'PM':'AM'}</span></div>
                          </div>
                      );
                  })}
              </>
      ...
      return (
          <>
              {['eidFitr', 'eidAdha'].map(eidKey => {
                  ...
                  return (
                      <div key={eidKey}>
                          <div className="mt-6 mb-2 flex items-center gap-2 px-1"><i className="fas fa-star text-xs text-amber-500"></i><h3 className="text-xs font-bold text-amber-600 uppercase tracking-widest">{specialPrayersList.find(s=>s.id===eidKey).name}</h3></div>
                          {sublist.map(m => {
                              ...
                              return (
                                  <div key={m.id} onClick={() => { setSelectedMosqueId(m.id); setActiveModal('detail'); }} className="flex justify-between items-center bg-white dark:bg-gray-800 px-4 py-2.5 rounded-xl shadow-sm border-l-[3px] border-amber-500 mb-1 animate-card cursor-pointer">
                                      <div className="flex-1 mr-4 flex items-center gap-2"><i className="fas fa-mosque text-[10px] text-amber-400"></i><div><h4 className="font-bold text-sm dark:text-white leading-tight">{m.name}</h4><p className="text-[9px] text-gray-400">{m.area}</p></div></div>
                                      <div className="text-right font-mono font-bold text-lg dark:text-white">{parseInt(h)%12||12}:{mins}<span className="text-[9px] ml-1">{h>=12?'PM':'AM'}</span></div>
                                  </div>
                              );
                          })}
                      </div>
              ...
              {sequence.map((pid, idx) => {
                  ...
                  return (
                      <div key={pid}>
                          <div className="mt-6 mb-2 flex items-center gap-2 px-1"><i className={`fas ${pObj.icon} text-xs ${idx===0?'text-brand-500':'text-gray-400'}`}></i><h3 className={`text-xs font-bold uppercase tracking-widest ${idx===0?'text-brand-600 dark:text-brand-400':'text-gray-400'}`}>{pObj.name}</h3></div>
                          {sublist.map(m => {
                              const t = m.timings[pid]; const [h, mins] = t.time.split(':'); 
                              const rem = idx === 0 ? getTimeRemaining(t.time, pid) : ''; 
                              ...
                              return (
                                  <div key={m.id} onClick={() => { setSelectedMosqueId(m.id); setActiveModal('detail'); }} className={`cursor-pointer flex justify-between items-center bg-white dark:bg-gray-800 px-4 py-2.5 rounded-xl shadow-sm border-l-[3px] ${idx===0?(passed?'border-gray-300':'border-brand-500'):'border-gray-200'} mb-1 transition-all ${passed?'opacity-40 grayscale':''}`}>
                                      <div className="flex-1 flex items-center gap-2">
                                          ...
                                          <div>
                                              <h4 className="font-bold text-sm dark:text-white leading-tight">{m.name}</h4>
                                              <p className="font-bold text-[9px] text-gray-400 font-medium">{m.area}</p>
                                              {taraweehData && <span className="inline-block text-[8px] font-bold text-amber-800 dark:text-amber-100 mt-1 px-1.5 py-0.5 rounded bg-amber-400 dark:bg-amber-600 shadow-sm uppercase">Tarāweeḥ: {taraweehData} P</span>}
                                          </div>
                                      </div>
                                      <div className="text-right">
                                          <div className="font-mono font-bold text-lg dark:text-white">{parseInt(h)%12||12}:{mins}<span className="text-[9px] ml-1">{h>=12?'PM':'AM'}</span></div>
                                          {t.lastUpdated && !t.fixed && <div className="font-narrow text-[8px] text-gray-400 mt-0.5">({formatUpdateDate(t.lastUpdated)})</div>}
                                          {idx===0 && <div className={`text-[9px] font-bold ${passed?'text-gray-400':'text-brand-600 bg-brand-50 dark:bg-brand-900/40'} px-2 rounded`}>{rem}</div>}
                                      </div>
                                  </div>
                              );
                          })}
      ...
      return activeMosques.map(m => {
          ...
          specialPrayersList.forEach(sp => { 
              ...
              if(appSettings[sp.mode] === true && m.timings && m.timings[sp.id]?.time) 
                  specHTML.push(<div key={sp.id} className="flex justify-between px-2 py-1 bg-amber-50 dark:bg-amber-900/10 rounded mb-1 text-[9px] font-bold"><span className="text-amber-700">{sp.name}</span><span className="text-amber-800 dark:text-white" dangerouslySetInnerHTML={{__html: formatTime12(m.timings[sp.id].time, sp.id)}}></span></div>); 
          });
              ...
              <div key={m.id} className="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-sm border-l-[3px] border-r-[3px] border-l-brand-500 border-r-gray-100 dark:border-r-gray-700 mb-3 animate-card">
                  <div className="flex justify-between items-start mb-3">
                      <div className="cursor-pointer flex items-start gap-3" onClick={() => { setSelectedMosqueId(m.id); setActiveModal('detail'); }}>
                          ...
                          <div><h2 className="text-sm font-bold text-gray-800 dark:text-white leading-tight">{m.name}</h2><p className="font-bold text-[10px] text-gray-400 font-medium">{m.area}</p></div>
                      </div>
                  ...
                  <div className="grid grid-cols-5 gap-0.5 text-center border-t border-gray-50 dark:border-gray-700 pt-3 mb-3">
                      {prayersList.map(p => {
                          ...
                          if(!t) return <div key={p.id} onClick={() => tryAction('edit',()=>openEditTiming(m.id))} className="opacity-30 cursor-pointer"><div className="font-narrow text-[8px] font-bold text-brand-500/80 mb-1 flex flex-col items-center gap-0.5 uppercase"><i className={`fas ${p.icon} text-[10px]`}></i> {p.name.slice(0,5)}</div>-</div>;
                          ...
                          return (
                              <div key={p.id} onClick={() => tryAction('edit',()=>openEditTiming(m.id))} className="cursor-pointer py-1">
                                  <div className="text-[8px] font-bold text-brand-500/80 mb-1 flex flex-col items-center gap-0.5 uppercase"><i className={`fas ${p.icon} text-[10px]`}></i> {p.name.slice(0,5)}</div>
                                  <div className="font-mono text-sm font-bold dark:text-white">{parseInt(h)%12||12}:{mins}<span className="text-[7px] block">{h>=12?'PM':'AM'}</span></div>
                                  {m.timings[p.id].lastUpdated && !m.timings[p.id].fixed && <div className="font-bold text-[7px] text-gray-400 mt-0.5">({formatUpdateDate(m.timings[p.id].lastUpdated)})</div>}
                                  {taraweehData && <div className="mx-auto mt-1 px-1 py-0.5 rounded bg-amber-400 dark:bg-amber-600 text-[6px] font-bold text-amber-900 dark:text-amber-50 w-fit">{taraweehData}P</div>}
                              </div>
                          );
                      })}
                  </div>
                  ...
                  <div className="flex border-t border-gray-100 dark:border-gray-700 pt-2">
                      <button onClick={(e) => { e.currentTarget.parentElement.nextSibling.classList.toggle('hidden'); }} className="flex-1 text-[10px] font-bold text-gray-400 uppercase">Notes <i className="fas fa-chevron-down"></i></button>
                      {hasLink ? 
                          <a href={m.locationLink} target="_blank" rel="noreferrer" className="flex-1 text-[10px] font-bold text-brand-600 text-center uppercase">Maps <i className="fas fa-location-arrow"></i></a> :
                          <span className="flex-1 text-[10px] font-bold text-gray-300 text-center uppercase cursor-not-allowed">Maps <i className="fas fa-location-arrow"></i></span>
                      }
                  </div>
                  <div className="hidden bg-gray-50 dark:bg-gray-700/30 p-3 text-[10px] border-t mt-2 dark:text-gray-300 whitespace-pre-line">{m.address||'No notes.'}</div>
              </div>
          );
      });
  };
    ...
    <div className="bg-gray-50 dark:bg-gray-900 text-gray-800 dark:text-gray-100 font-sans transition-colors duration-200 min-h-screen">
                ...
                {currentUser ? (
                    <div>
                        <div className="flex items-center gap-3 mb-3">
                            ...
                            <div>
                                <p className="text-xs font-bold text-gray-800 dark:text-white truncate max-w-[140px]">{currentUser.displayName}</p>
                                <p className="text-[9px] font-bold uppercase tracking-wider text-brand-600 bg-brand-50 dark:bg-brand-900/30 px-1.5 py-0.5 rounded inline-block">{userRole}</p>
                            </div>
                        </div>
                        <button onClick={() => signOut(auth)} className="text-xs font-bold text-red-500 hover:text-red-700">Log Out</button>
                    </div>
                ) : (
                    <button onClick={() => signInWithPopup(auth, provider)} className="w-full flex items-center justify-center gap-2 py-3 bg-gray-900 text-white rounded-xl text-xs font-bold shadow-lg hover:bg-gray-800 transition-colors">
                        <i className="fab fa-google"></i> Sign in to Sync
                    </button>
                )}
            </div>

            <h2 className="text-2xl font-serif font-bold mb-1 text-brand-600 dark:text-brand-400">Settings</h2>
            <p className="text-xs text-gray-400 mb-8 uppercase tracking-widest">Preferences</p>
            
            <div className="space-y-6 flex-1">
                <div className="flex items-center justify-between">
                    <span className="font-bold text-sm">Dark Mode</span>
                    <div className="relative inline-block w-10 align-middle select-none">
                        <input type="checkbox" id="theme_toggle" checked={appSettings.theme === 'dark'} onChange={toggleTheme} className="toggle-checkbox absolute block rounded-full bg-white border-4 appearance-none cursor-pointer border-gray-300"/>
                        <label htmlFor="theme_toggle" className="toggle-label block overflow-hidden rounded-full bg-gray-300 cursor-pointer"></label>
                    </div>
                </div>
                <hr className="border-gray-100 dark:border-gray-700" />
                <div className={userRole !== 'admin' ? 'restricted' : ''}>
                    <h3 className="text-[10px] font-bold uppercase text-gray-400 mb-4 tracking-wider flex justify-between items-center">Seasonal Visibility <i className="fas fa-lock text-[9px] opacity-30"></i></h3>
                    <div className="space-y-4 text-sm font-medium">
                        {['ramadan', 'eidFitr', 'eidAdha', 'qiyam', 'lateIsha'].map(key => (
                            <div key={key} className="flex justify-between items-center">
                                <span className="capitalize">{key.replace(/([A-Z])/g, ' $1').trim()}</span>
                                <input type="checkbox" checked={appSettings[key]} onChange={() => toggleMode(key)} className="accent-brand-600 w-4 h-4"/>
                            </div>
                        ))}
                    </div>
                </div>
        ...
        <div className={`fixed top-0 w-full z-40 bg-white/95 dark:bg-gray-800/95 backdrop-blur-md shadow-sm border-b border-gray-100 dark:border-gray-700 transition-all ${viewMode === 'next' ? (isFriday ? 'h-[138px]' : 'h-[115px]') : (viewMode === 'info' ? 'h-[50px]' : 'h-[95px]')}`}>
            <div className="flex flex-col items-center justify-center pt-2 pb-1 relative h-[50px]">
                ...
                <p className="text-[9px] text-gray-400 font-medium -mt-0.5">by @aekae47</p>
            </div>

            {viewMode === 'next' && (
                <div className="w-full pb-2">
                    <div className="flex flex-col items-center justify-center py-1 cursor-pointer" onClick={() => setActiveModal('city')}>
                        <div className="text-4xl font-mono font-bold text-gray-800 dark:text-white tracking-tighter leading-none flex items-baseline gap-1" dangerouslySetInnerHTML={{__html: currentTimeDisplay}}></div>
                        <div className="flex items-center gap-1.5 mt-0.5 text-gray-400 dark:text-gray-500">
                            ...
                            <span className="text-[10px] font-bold uppercase tracking-[0.2em]">{appSettings.city.toUpperCase()}</span>
                        </div>
                        {isFriday && <div className="mt-1 bg-emerald-50 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-400 px-3 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-[0.15em] border border-emerald-100 dark:border-emerald-800/50 shadow-sm mx-auto w-fit">It's Friday! ✨</div>}
                    </div>
                </div>
            )}
            ...
            {(viewMode === 'list' || viewMode === 'next') && viewMode !== 'info' && (
                <div className={`px-4 pt-1 pb-2 ${viewMode === 'next' ? 'hidden' : ''}`}>
                    <div className="relative">
                        ...
                        <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search Masājid..." className="w-full bg-gray-100 dark:bg-gray-700/50 border-none rounded-lg py-2 pl-9 pr-4 text-xs font-bold outline-none dark:text-white focus:ring-2 focus:ring-brand-500" />
                    </div>
                </div>
            )}
        </div>
        ...
        <div style={{ paddingTop: viewMode === 'next' ? (isFriday ? '138px' : '115px') : (viewMode === 'info' ? '50px' : '95px') }} className="max-w-md mx-auto px-3">
            ...
            {viewMode !== 'info' && (
                <div className="w-full bg-transparent py-2">
                    <div className="flex overflow-x-auto no-scrollbar gap-2">
                        {['All', 'Favorites', ...Object.keys(personalLists).filter(l => l !== 'Favorites')].map(list => (
                            <button key={list} onClick={() => setCurrentList(list)} className={`px-3 py-1.5 rounded-lg text-[10px] font-bold transition-all whitespace-nowrap border ${currentList === list ? 'bg-brand-600 text-white border-brand-600 shadow-md' : 'bg-white dark:bg-gray-700 text-gray-500 border-gray-200 dark:border-gray-600'}`}>{list}</button>
                        ))}
            ...
            <div className="pb-24 min-h-screen">
                {mosques.length === 0 ? (
                    <div className="text-center mt-32 text-gray-400 flex flex-col items-center"><i className="fas fa-circle-notch fa-spin text-2xl mb-3 text-brand-300"></i><p className="text-xs font-bold uppercase tracking-widest opacity-50">Starting...</p></div>
                ) : (
                    <>
                        ...
                        {viewMode === 'info' && (
                            <div className="animate-card max-w-sm mx-auto pt-6 space-y-8 pb-10">
                                {/* Omitted full raw HTML of Info to save space, but keeping standard structure */}
                                <div className="space-y-3 pt-4"> 
                                    <button onClick={() => setActiveModal('contact')} className="flex items-center justify-center gap-3 w-full py-4 bg-gray-800 text-white rounded-2xl shadow-lg hover:brightness-110 transition-all">
                                        <i className="fas fa-envelope text-xl"></i><span className="text-sm font-bold uppercase tracking-wider">Contact Us</span>
                                    </button>
                                </div>
                            </div>
                        )}
        ...
        {/* BOTTOM NAVIGATION */}
        <div className="fixed bottom-0 w-full bg-white/95 dark:bg-gray-800/95 backdrop-blur-md border-t border-gray-200 dark:border-gray-700 p-2 flex justify-around z-40 pb-4">
        ...
        {activeModal === 'personalList' && (
            <div className="fixed inset-0 z-[100] bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
                <div className="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-sm p-6 shadow-2xl animate-card">
            ...
            <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
                <div className="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-sm max-h-[85vh] shadow-2xl flex flex-col animate-card overflow-hidden">
                    <div className="p-5 border-b border-gray-100 dark:border-gray-700 bg-gray-50/50 flex justify-between items-start">
                        <div className="flex items-start gap-3"><i className="fas fa-mosque text-brand-600 text-xl mt-1"></i><div><h2 className="text-xl font-serif font-bold dark:text-white leading-tight">{selectedMosqueDetail.name}</h2><p className="text-xs font-bold text-gray-500 mt-1">{selectedMosqueDetail.area}</p></div></div>
                        <div className="flex gap-2">
                            <button onClick={() => tryAction('edit', () => openMosqueModal(selectedMosqueId))} className="w-8 h-8 rounded-full bg-brand-50 text-brand-600 flex items-center justify-center hover:bg-brand-100"><i className="fas fa-pencil-alt text-xs"></i></button>
                    ...
                    <div className="flex-1 overflow-y-auto p-5">
                        <div className="space-y-2 mb-6 cursor-pointer" onClick={() => tryAction('edit', () => openEditTiming(selectedMosqueId))}>
                            {['fajr', 'zuhr', 'jumma', 'asr', 'isha'].map(pid => {
                                ...
                                return (
                                    <div key={pid} className={`flex justify-between items-center p-3.5 rounded-xl mb-2 bg-gray-50 border ${!hasTime ? 'opacity-40' : ''}`}>
                                        <span className="font-bold text-sm text-gray-600 flex items-center gap-2"><i className={`fas ${pObj.icon} text-xs w-4`}></i> {pObj.name}</span>
                                        <div className="text-right flex flex-col items-end">
                                            <span className={hasTime ? 'font-mono text-lg font-bold text-gray-800' : 'text-[10px] text-gray-400 italic'} dangerouslySetInnerHTML={{__html: hasTime ? formatTime12(data.time, pid) : '(Timing not entered)'}}></span>
                                            {taraweeh && <div className="text-[9px] font-bold mt-1 px-2 py-0.5 rounded bg-amber-400">Tarāweeḥ: {taraweeh} Pārahs</div>}
                                        </div>
                                    </div>
                                );
                            })}
        ...
        {activeModal === 'timing' && selectedMosqueDetail && (
            <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
                <div className="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-sm p-5 shadow-2xl animate-card max-h-[90vh] overflow-y-auto no-scrollbar">
                    <div className="flex justify-between items-center mb-5 pb-3 border-b"><h3 className="text-lg font-serif font-bold">{selectedMosqueDetail.name}</h3><button onClick={() => setActiveModal(null)} className="text-gray-400"><i className="fas fa-times"></i></button></div>
                            ...
                            return (
                                <div key={p.id} className="border-b pb-3 mb-3">
                                    <div className="flex items-center gap-3 mb-2">
                                        ...
                                        <input type="time" value={val} onChange={e => setTimingFormData({...timingFormData, [p.id]: {...timingFormData[p.id], time: e.target.value}})} className="flex-1 bg-gray-50 border rounded-xl px-3 py-2 font-mono font-bold" />
                                    </div>
            ...
            <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
                <div className="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-sm p-6 shadow-2xl animate-card">
                    ...
                    <div className="space-y-4">
                        <input type="text" value={mosqueFormData.name} onChange={e=>setMosqueFormData({...mosqueFormData, name: e.target.value})} className="w-full bg-gray-50 border rounded-lg px-3 py-2 font-bold" placeholder="Masjid-e-..." />
        ...
        {/* Toast */}
        <div className={`fixed bottom-24 left-1/2 transform -translate-x-1/2 bg-gray-900 text-white px-5 py-2.5 rounded-full text-xs font-bold shadow-2xl transition-all duration-300 z-50 pointer-events-none ${toastVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
